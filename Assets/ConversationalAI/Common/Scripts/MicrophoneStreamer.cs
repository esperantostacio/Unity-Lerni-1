...
public class MicrophoneStreamer : MonoBehaviour
{
    ...
    private void Start()
    {
        // Note: On some platforms (Android/Quest) microphone devices may be unavailable
        // until the user grants mic permission. Don't permanently disable here.
        if (Microphone.devices.Length == 0)
            Debug.LogWarning("MicrophoneStreamer: no microphone devices yet (permission not granted?).");
        else
    ...
    public void StartStreaming()
    {
        ...
        // Ensure mic permission (especially important on Android/Quest).
        if (!HasMicPermission())
        {
        ...
        // Ensure we have a device selection.
        if (string.IsNullOrEmpty(_micDevice))
        {
            if (Microphone.devices.Length == 0)
            {
                Debug.LogError("MicrophoneStreamer: no microphone devices available.");
                return;
            }
...
#else
        // Desktop typically does not require explicit runtime permission via Unity API.
        return true;
#endif
    }
    ...
    {
#if UNITY_ANDROID || UNITY_IOS
        Debug.LogWarning("MicrophoneStreamer: requesting microphone permission...");
        ...
        {
            Debug.LogError("MicrophoneStreamer: microphone permission denied.");
            yield break;
        }
