...
namespace AutoLOD.MeshDecimator
{
    public class AutoLODWindow : EditorWindow
    {
        ...
        public void OnEnable()
        {
            ...
            {
                string componentName = "HDAdditionalLightData";
                string assemblyName = "UnityEngine.Rendering.HighDefinition";
                Assembly assembly = Assembly.Load(assemblyName);

                if (assembly != null)
                {
                    Type componentType = assembly.GetType(componentName);
                    if (componentType != null)
                    {
                        foreach (Light light in previewRender.lights)
                        {
                            if (light.GetComponent(componentType) == null)
                                light.gameObject.AddComponent(componentType);
                        }
                    }
                }
            }
        ...
        void OnGUI()
        {
            ...
            if (true || centeredStyle == null)
            {
                centeredStyle = EditorStyles.boldLabel;
            ...
            GUILayout.Label(Resources.Load<Texture>("Logo_AutoLOD"), centeredStyle, GUILayout.Height(32f));
            ...
            EditorGUILayout.LabelField("Targets", centeredStyle);
            EditorGUILayout.BeginHorizontal();
            ...
            EditorGUILayout.LabelField("Common Settings", centeredStyle);
            ...
            if (showPreview)
            {
                ...
                GUILayout.Label("Preview Area", centeredStyle);
                EditorGUILayout.Separator();
                EditorGUILayout.BeginHorizontal();
            ...
            GUILayout.Label("v5.4.0 ", smallFont, GUILayout.Height(8));
            GUILayout.BeginHorizontal();
            GUILayout.Label("© 2020-2025 ", smallFont, GUILayout.Height(8));
